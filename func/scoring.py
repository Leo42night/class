...
def _parse_score_txt(code_class, tugas_ke):
    ...
    with open(f"assign_score/{code_class}-{tugas_ke}-score.txt", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
...
def _get_tab_name(service_sheet, spreadsheet_id, tugas_ke):
    """
...
def _fetch_sheet_names(service_sheet, spreadsheet_id, range_sheet):
    """Ambil daftar nama dari sheet, return list of rows."""
    result = service_sheet.spreadsheets().values().get(
        spreadsheetId=spreadsheet_id,
        range=range_sheet
    ).execute()
...
def _print_table(table_data):
    ...
    for item in table_data:
        note_short = (item["notes"][:37] + "..") if len(item["notes"]) > 37 else item["notes"]
        print(template.format(item["row"], item["name"][:25], note_short, item["score"]))
...
def run_scoring(course_id, coursework_id, spreadsheet_id, name_class, tugas_ke):
    ...
    service_sheet = _build_sheet_service()
    tab_name = _get_tab_name(service_sheet, spreadsheet_id, tugas_ke)
    range_sheet = f"{tab_name}!F2:F{2 + 40}"
    names_rows = _fetch_sheet_names(service_sheet, spreadsheet_id, range_sheet)
