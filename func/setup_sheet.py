# Menu Fitur setup.py (bukan main.py)
# 1. Sheet Student Name (from classroom)
# >> manual: pair with codename (periksa aturan cara buat codename)
# 2. Sheet Link Github Profile (take from submission classroom)
import sys
import os
...
def _apply_github_cell_formats(
    service_sheet, spreadsheet_id: str, sheet_id: int, rows: list[dict]
):
    """
...
def setup_nama(course_id: str, spreadsheet_id: str):
    ...
    print(
        f"✅ {len(students)} nama berhasil ditulis ke '{TAB_NAME}'!{NAME_COL}{ROW_START}:{NAME_COL}{range_end}."
    )
...
def setup_github_profile(course_id: str, spreadsheet_id: str):
    ...
    print(
        f"  Link GitHub ditemukan: {len(github_map)} dari {len(submissions)} submisi."
    )
    ...
    existing_col = [
        row[0].strip() if row else "" for row in existing_resp.get("values", [])
    ]
    ...
    confirm = (
        input(f"Tulis ke Spreadsheet (kolom {REPO_COL} tab '{TAB_NAME}')? (y/n) >> ")
        .strip()
        .lower()
    )
    if confirm != "y":
    ...
    if empty_requests:
        service_sheet.spreadsheets().batchUpdate(
            spreadsheetId=spreadsheet_id, body={"requests": empty_requests}
        ).execute()

    print(
        f"✅ {filled} GitHub profile berhasil ditulis ke '{TAB_NAME}'!{REPO_COL} (underline, biru)."
    )
